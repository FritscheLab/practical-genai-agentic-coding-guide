...
# Usage: Rscript scripts/r/simulate_ehr_data.R [options]
# Example:
...
#   - Packages: optparse
#   - Setup: See docs/reference/synthetic-data.md; install dependencies before running.
#     For an existing renv project, run renv::restore() from its project root.
###############################################################################

#------------------------------------------------------------------------------
# Library Management: Check and load installed packages
#------------------------------------------------------------------------------
required_packages <- c("optparse") # For handling command-line arguments

# Keep environment setup separate from analysis execution.
...
if (length(missing_packages) > 0) {
    stop(
        ...
        "See docs/reference/synthetic-data.md for dependency setup. ",
        "If your project has renv.lock, run renv::restore() from its project root.",
        call. = FALSE
    )
}
...
person_id_length    <- 12 # Length of synthetic person_id strings
encounter_id_length <- 12 # Length of synthetic encounter_id strings
...
dict_descriptions_ehr <- c(
    ...
    "Randomly generated synthetic person identifier (consistent across records).",
    "Randomly generated synthetic encounter identifier (unique per record).",
...
dict_descriptions_demo <- c(
    ...
    "Synthetic deceased indicator (Yes/No).",
    paste("Synthetic race label (NA for:", paste(na_race_eth_values, collapse=", "), ")."),
    paste("Synthetic ethnicity label (NA for:", paste(na_race_eth_values, collapse=", "), ")."),
    ...
    "Synthetic sex/gender label.",
    "Synthetic marital status.",
