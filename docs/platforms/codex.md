...
1. Complete the [setup for your language path](../quickstart.md), then launch `codex` from the example repository root or open the example repository in your Codex client. This keeps the agent’s work in the project you just tested.
...
Open `/model` in the CLI to choose an available model and, where supported, its reasoning effort. For a new session, `codex --model MODEL_ID` selects the model you name; replace `MODEL_ID` with an identifier your provider supports. A `model` setting can supply a personal or trusted-project default. The example repository leaves it unset so you and a labmate can use different models with the same task. [Model controls](https://learn.chatgpt.com/docs/developer-commands), [configuration options](https://learn.chatgpt.com/docs/config-file/config-basic).
...
In the CLI, `/plan` starts planning; you can include a request, such as `/plan Read the exclusion-report brief and propose the smallest change.` Use that discussion to explain the counts in the six-row example before coding. The desktop app also documents `/plan`, with its own command menu. [CLI planning command](https://learn.chatgpt.com/docs/developer-commands#switch-to-plan-mode-with-plan), [desktop commands](https://learn.chatgpt.com/docs/reference/slash-commands).
...
From the new repository's root, replace the example source path below with your local checkout of the example repository. These commands leave existing files in place. If you already have a setup, merge the settings and procedures you want to adopt in your editor.
...
cp -n "$example_repo/.codex/agents/pipeline-reviewer.toml" .codex/agents/
cp -Rn "$example_repo/.agents/skills/pipeline-review" .agents/skills/
```
